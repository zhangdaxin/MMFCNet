...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, features_batch_cnt: torch.Tensor,
        ...
        assert features.shape[0] == features_batch_cnt.sum().cpu(), \
            'features: %s, features_batch_cnt: %s' % (str(features.shape), str(features_batch_cnt))
        assert idx.shape[0] == idx_batch_cnt.sum().cpu(), \
...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        # xyz = xyz.to(device)
        # print(xyz.shape[0].is_cuda,xyz_batch_cnt.sum().is_cuda)
        
        assert xyz.shape[0] == xyz_batch_cnt.sum().cpu(), 'xyz: %s, xyz_batch_cnt: %s' % (str(xyz.shape), str(new_xyz_batch_cnt))
        assert new_xyz.shape[0] == new_xyz_batch_cnt.sum().cpu(), \
