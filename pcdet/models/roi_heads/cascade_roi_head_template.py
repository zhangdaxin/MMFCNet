...
class CascadeRoIHeadTemplate(nn.Module):
    ...
    def proposal_layer(self, batch_dict, nms_config):
        ...
        for index in range(batch_size):
            ...
            rois[index, :len(selected), :] = box_preds[selected].squeeze()
            # box_preds_selected = box_preds[selected]
            # box_preds_selected = box_preds_selected.view(len(selected), -1)[0]
            # rois[index, :len(selected), :] = box_preds_selected
            roi_scores[index, :len(selected)] = cur_roi_scores[selected].squeeze()
            roi_labels[index, :len(selected)] = cur_roi_labels[selected].squeeze()
    ...
    def get_loss(self, tb_dict=None):
        ...
        tb_dict['rcnn_loss'] = torch.tensor(rcnn_loss.item())
        
        tb_dict['rpn_loss_loc'] = torch.tensor(tb_dict['rpn_loss_loc'])
        tb_dict['rpn_loss_cls'] = torch.tensor(tb_dict['rpn_loss_cls'])
        tb_dict['rpn_loss_dir'] = torch.tensor(tb_dict['rpn_loss_dir'])
        tb_dict['rpn_loss'] = torch.tensor(tb_dict['rpn_loss'])
        tb_dict['point_loss_cls'] = torch.tensor(tb_dict['point_loss_cls'])
        tb_dict['point_pos_num'] = torch.tensor(tb_dict['point_pos_num'])
        tb_dict['point_loss_part'] = torch.tensor(tb_dict['point_loss_part'])
